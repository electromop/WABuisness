...
def choose_category(notification: Notification):
    match notification.message_text:
        ...
        case _:
            ...
            notification.answer("Выберите категорию материала:\n\n"
                                "1. Срочно и важно\n"
...
#@bot.router.message(command="test")
#def test(notification: Notification):
#    res = disk.listdir("disk:/Загрузки/Обучающие материалы для мерчандайзера-новичка/")
#    for i in res:
#        print(i)
#    #ans = [i["name"] for i in res if i.get("type") != "dir"]
#    #print(ans)
...
def key_word_handler(notification: Notification) -> None:
    ...
    if SyncORM.check_key(key_word):
        phone = sender.split("@")[0]
        SyncORM.update_chat(chat, phone)
